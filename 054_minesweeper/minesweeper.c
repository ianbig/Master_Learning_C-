...
board_t * makeBoard(int w, int h, int numMines) {
  if (w < 0 || h < 0 || numMines < 0) {
    fprintf(stderr, "negative value as parameters is banned\n");
    exit(EXIT_FAILURE);
  }

  if (numMines > w * h) {
    fprintf(stderr, "Error: numMines is higher than w * h\n");
    exit(EXIT_FAILURE);
  }
  // spcify height and width and mines in board_t
  board_t * ret = malloc(sizeof(*ret));
  if (ret == NULL) {
    fprintf(stderr, "Error: heap size is not enough for malloc\n");
    exit(EXIT_FAILURE);
  }

  ret->height = h;
  ret->width = w;
  ret->totalMines = numMines;
  ret->board = malloc(sizeof(*(ret->board)) * h);
  if (ret->board == NULL) {
    fprintf(stderr, "Error: heap size not enough for malloc\n");
    exit(EXIT_FAILURE);
  }

  for (int i = 0; i < h; i++) {
    ret->board[i] = malloc(sizeof(**(ret->board)) * w);
    if (ret->board[i] == NULL) {
      fprintf(stderr, "Error: heap size not enough for malloc\n");
      exit(EXIT_FAILURE);
    }
    for (int j = 0; j < w; j++) {
      ret->board[i][j] = UNKNOWN;
    }
  }
  // call addRandomMine to place mines with for loop
